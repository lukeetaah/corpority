...
type PendingConsequence = { turns: number; title: string; message: string; changes: Changes };
type Game = { position: number; year: number; turn: number; stats: Record<Stat, number>; items: string[]; flags: string[]; log: string[]; skippedTurns: number; seenEventIds: string[]; recentEventTitles: string[]; pending: PendingConsequence[]; ended?: "won" | "burnout" };
type Resolution = { title: string; message: string; changes: Changes; skippedTurns?: number; item?: string };
...
function createGame(locale: Locale): Game {
  return { position: 0, year: 1, turn: 1, stats: { salary: 38, savings: 14, health: 76, energy: 72, motivation: 68, reputation: 18, influence: 4 }, items: [], flags: [], skippedTurns: 0, seenEventIds: [], recentEventTitles: [], pending: [], log: [getGameData(locale).text.startLog] };
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (savedGame) { try { const parsed = JSON.parse(savedGame); setGame({ ...createGame(savedLanguage === "es-AR" ? "es-AR" : "en"), ...parsed, skippedTurns: parsed.skippedTurns ?? 0, seenEventIds: parsed.seenEventIds ?? [], recentEventTitles: parsed.recentEventTitles ?? [], pending: parsed.pending ?? [] }); } catch {} }
  ...
  function apply(changes: Changes, message: string, choice?: Choice) {
    setGame(current => {
      ...
      const pending = choice?.delayed ? [...current.pending, choice.delayed] : current.pending;
      ...
      const ended = nextStats.health === 0 ? "burnout" : current.year >= CAREER_YEARS && nextStats.savings >= 50 && nextStats.health >= 35 ? "won" : undefined;
      return { ...current, stats: nextStats, flags, items: item, pending, skippedTurns, ended, log: [`${message}${penalty}${delayed}`, ...current.log].slice(0, 5) };
    });
  }
  function resolveChoice(choice: Choice) {
    ...
    setResolution({ title: choice.label, message: choice.consequence, changes: effective, skippedTurns: choice.skipTurns, item: choice.item });
    setActiveEvent(null);
  ...
  function roll() {
    ...
    if (due) {
      setGame(current => {
        ...
        for (const [key, value] of Object.entries(due.changes)) stats[key as Stat] = clamp(stats[key as Stat] + value!);
        return { ...current, stats, pending: current.pending.filter(consequence => consequence !== due), log: [`${due.title}: ${due.message} ${changesText(due.changes)}`, ...current.log].slice(0, 5) };
      });
      setActiveEvent({ id: `deferred-${due.title}`, title: due.title, description: due.message, category: isSpanish ? "Consecuencia" : "Consequence", changes: due.changes, choices: [{ label: isSpanish ? "Seguir" : "Continue", consequence: isSpanish ? "La agenda no espera." : "The calendar does not wait.", changes: {} }], rarity: "rare" });
      return;
    }
  ...
  return <main className="shell">
    ...
    {resolution && <aside className="resolution-push"><button onClick={() => setResolution(null)} aria-label={isSpanish ? "Cerrar resultado" : "Close result"}>×</button><span>{isSpanish ? "DECISIÓN REGISTRADA" : "DECISION LOGGED"}</span><b>{resolution.title}</b><p>{resolution.message}</p><div className="impact-chips">{Object.entries(resolution.changes).map(([key, value]) => <em className={value! > 0 ? "gain" : "loss"} key={key}>{value! > 0 ? "+" : ""}{value} {stats[key as Stat].label}</em>)}{resolution.skippedTurns ? <em className="loss">{isSpanish ? `Prenda · ${resolution.skippedTurns} turno` : `Forfeit · ${resolution.skippedTurns} turn`}</em> : null}{resolution.item ? <em className="gain">+ {resolution.item}</em> : null}</div></aside>}
