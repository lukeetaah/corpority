...
function choicesForScenario(locale: Locale, event: CorporateEvent): Choice[] {
  ...
  const situationIndex = Number(parts[parts.length - 2]);
  const key = scenarioGroups[situationIndex] ?? "workload";
...
export default function Home() {
  ...
  function roll() {
    ...
    window.setTimeout(() => {
      ...
      window.setTimeout(() => setDicePush(null), 980);
      window.setTimeout(() => setActiveEvent(event), 820);
    }, 520);
  }
